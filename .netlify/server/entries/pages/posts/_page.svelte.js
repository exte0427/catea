...
const css$1 = {
  code: "#inner.svelte-17qc2om{border-width:0px;width:100%;background-color:rgb(87, 81, 74);margin:10px;margin-bottom:0;padding:10px;cursor:pointer;box-shadow:5px 5px 0 rgba(50, 49, 47, 0.654);color:white;border-radius:5px;transition:0.3s}#inner.svelte-17qc2om:hover{box-shadow:0px 0px 0 rgba(50, 49, 47, 0.654);transform:translate(5px, 5px)}#title.svelte-17qc2om{font-size:1.5em}#category.svelte-17qc2om{background-color:white;color:rgb(87, 81, 74);padding:2px;border-radius:5px}",
  map: null
};
const PostCard = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `<button id="inner" class="svelte-17qc2om"><strong id="title" class="svelte-17qc2om">${escape(postData.title)}</strong> <p>${escape(PageModule.getDate(postData.date))} <strong id="category" class="svelte-17qc2om">${escape(postData.category)}</strong></p> </button>`;
});
const css = {
